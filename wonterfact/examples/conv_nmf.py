...
def make_deconv_tree():
    ...
    impulse_kyx[impulse_kyx < 150] = 0
    impulse_kyx[impulse_kyx >= 150] = 200
    ...
    leaf_kernel = wtf.LeafDirichlet(
        ...
        index_id=("k", "y", "x"),
        norm_axis=(1, 2),
        tensor=npr.dirichlet(np.ones(dim_y1 * dim_x1), size=dim_k).reshape(
            (dim_k, dim_y1, dim_x1)
        ),
        prior_shape=100,
    )
    ...
    leaf_kernel.new_child(mul_image)
    leaf_impulse.new_child(mul_image)
    obs = wtf.PosObserver(
        name="image",
        index_id="yx",
        tensor=image_yx,
