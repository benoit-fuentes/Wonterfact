...
# Relative imports
from . import _core_nodes, buds, utils
from .glob_var_manager import glob


class LeafDirichlet(_core_nodes.DynNodeData, _core_nodes.ChildNode):
    """
    ...
    def _initialization(self):
        ...
        if (
            self.update_period == 0 or self._inference_mode in ("EM", "VB-MCMC")
        ) and not glob.xp.allclose(self.tensor.sum(axis=self.norm_axis), 1):
            msg = "Please provide a well normalized tensor"
            raise ValueError(msg)

    def _update_tensor(
        self,
        update_type: Literal[
            "no_update_for_leaves", "parabolic", "regular", "simulate"
        ] = "regular",
        update_param=None,
    ):
        ...
        if update_type == "no_update_for_leaves":
            ...
            self._regular_update_tensor_no_norm()
            self._normalize_tensor(vbem_mode=(self._inference_mode == "VBEM"))
            if self._inference_mode == "VB-MCMC":
                self.update_sufficient_statistic()
        elif update_type == "simulate":
            self.tensor[...] = self.prior_shape
            self.tensor[...] = glob.xp.random.gamma(self.tensor, 1)
            self._normalize_tensor(vbem_mode=False)
        else:
            msg = "Unknown `update_type`"
            raise ValueError(msg)

    def _normalize_tensor(self, vbem_mode: bool = False):
        norm_tensor = self.tensor.sum(axis=self.norm_axis, keepdims=True)
        if vbem_mode:
            if self.constraint_coeffs is not None:
                raise NotImplementedError
            self.tensor[...] = utils.exp_digamma(self.tensor) / utils.exp_digamma(norm_tensor)
        elif self.constraint_coeffs is not None:
    ...
    def _regular_update_tensor_no_norm(self):
        ...
        if self._inference_mode == "VBEM":
            self.tensor *= tensor_update
            self.tensor += self.prior_shape
            self.posterior_shape[...] = self.tensor
        if self.inertia:
    ...
    def _parabolic_update(self, parabolic_param):
        ...
        if self._inference_mode == "EM" and (self.tensor <= self.min_val).any():
            self._clip_tensor_min_value()
            self._normalize_tensor(vbem_mode=False)
        ...
        if self._inference_mode == "VBEM":
            self._clip_tensor_min_value()
            self._normalize_tensor(vbem_mode=True)
